    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center mb-6">
                ...
                <button 
                   onClick={() => { setEditing({ icono: 'Package', color: '#06b6d4', orden: categorias.length }); setIsModalOpen(true); }}
                   className="flex items-center gap-3 bg-[#06b6d4]/10 border border-[#06b6d4]/30 hover:bg-[#06b6d4]/20 text-[#06b6d4] px-5 py-2.5 rounded-xl font-black text-[10px] tracking-widest uppercase transition-all shadow-[0_0_15px_rgba(6,182,212,0.1)]"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {categorias.map((cat) => {
                    ...
                    return (
                        <div key={cat.id} className="bg-[#161a23]/80 backdrop-blur-xl border border-white/10 p-5 rounded-2xl flex items-center justify-between group shadow-xl hover:border-[#06b6d4]/40 transition-all duration-300">
                            ...
                            <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-all pr-2 translate-x-2 group-hover:translate-x-0">
                                <button onClick={() => { setEditing(cat); setIsModalOpen(true); }} className="p-2.5 bg-white/5 hover:bg-[#06b6d4]/20 rounded-xl text-gray-400 hover:text-[#06b6d4] border border-white/5 transition-all">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-[#0D0E15]/95 backdrop-blur-xl z-[110] flex items-center justify-center p-4">
                    <div className="bg-[#161a23]/95 w-full max-w-lg rounded-[2.5rem] border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden p-10 relative">
                        <div className="absolute top-[-10%] right-[-20%] w-[350px] h-[350px] bg-[#06b6d4]/10 rounded-full blur-[120px] pointer-events-none" />

                        <div className="flex justify-between items-center mb-10 relative z-10 border-b border-white/5 pb-6">
                            <div>
                                <p className="text-[10px] font-sans tracking-[0.3em] uppercase text-[#06B6D4] font-black mb-2">ARCHIVE_MANAGER // BRANCH</p>
                        ...
                        <div className="space-y-8 relative z-10">
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <input 
                                        className="w-full bg-black/40 border border-white/5 rounded-xl py-4 px-5 text-white font-bold outline-none focus:border-[#06b6d4] transition-all shadow-inner"
                                ...
                                <div className="grid grid-cols-6 gap-3 max-h-[160px] overflow-y-auto pr-3 bg-black/30 p-5 rounded-2xl border border-white/5 custom-scrollbar shadow-inner">
                                    {Object.entries(ICON_MAP).map(([key, Icon]) => (
                                        <button 
                                            ...
                                            className={`p-3.5 rounded-xl flex items-center justify-center transition-all ${editing?.icono === key ? 'bg-[#06b6d4]/20 border border-[#06b6d4]/40 text-[#06b6d4] shadow-inner scale-110' : 'text-gray-500 hover:text-white hover:bg-white/10 border border-transparent'}`}
                                        >
                            ...
                            <button 
                                onClick={handleSave}
                                disabled={loading}
                                className="w-full bg-gradient-to-r from-[#06b6d4] to-[#00f0ff] py-5 rounded-2xl text-[#0D0E15] font-black uppercase tracking-[0.2em] shadow-[0_15px_30px_rgba(6,182,212,0.3)] transition-all disabled:opacity-50 hover:brightness-110 active:scale-[0.98] flex items-center justify-center gap-3 mt-4"
                            >
