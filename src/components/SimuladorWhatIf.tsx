...
const AnimatedCounter = ({ from, to, isDanger }: { from: number, to: number, isDanger: boolean }) => {
  ...
  const getColorClass = () => {
    if (isDanger) return "text-[#FF4A4A]";
    if (displayValue >= 90) return "text-bunker-limon";
    if (displayValue >= 70) return "text-white";
    return "text-bunker-mutado";
  };

  return (
    <div className="relative">
      <span className={`text-4xl md:text-5xl font-sans font-black tracking-tighter ${getColorClass()}`}>
        {displayValue}%
      </span>
    </div>
  );
...
export function SimuladorWhatIf({ puntoEstabilidad }: { puntoEstabilidad?: number }) {
  ...
  return (
    <div className="bg-bunker-bg p-6 lg:p-8 rounded-3xl border border-white/5 font-sans relative overflow-hidden selection:bg-bunker-limon selection:text-black">
      {/* Ambience background glow */}
      <div className="absolute left-0 right-0 top-0 -z-10 m-auto h-[310px] w-[310px] rounded-full bg-bunker-limonDim opacity-20 blur-[100px]" />

      <div className="relative z-10 flex flex-col md:flex-row gap-8 items-start mb-8 border-b border-white/5 pb-6">
        <div className="flex-1">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-bunker-limonDim text-bunker-limon text-[9px] font-black tracking-widest uppercase mb-4 border border-bunker-limon/20">
            <Beaker className="w-3.5 h-3.5" /> Laboratorio de Escenarios
          </div>
          <h2 className="text-2xl font-black text-white leading-tight uppercase font-sans">
            Motor de Simulación <span className="text-bunker-limon">Monte Carlo</span>
          </h2>
          <p className="text-bunker-mutado text-xs mt-3 leading-relaxed max-w-xl font-sans">
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        {/* Sliders Panel */}
        <div className="lg:col-span-5 bg-bunker-panel p-6 rounded-[2rem] border border-white/5 shadow-2xl">
          <div className="flex items-center gap-3 mb-6">
            <Activity className="w-4 h-4 text-bunker-limon" />
            <h3 className="text-xs uppercase tracking-widest font-black text-white font-sans">Variables de Estrés</h3>
          </div>
          ...
          <div className="space-y-8 font-sans">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="text-xs font-black uppercase text-bunker-mutado flex items-center gap-1.5">
                  Ventas Janlu Velas <Zap className="w-3.5 h-3.5 text-bunker-limon" />
                </label>
                <span className={`text-xs font-black font-mono ${ventasJanlu >= 0 ? 'text-bunker-limon' : 'text-[#FF4A4A]'}`}>
              ...
              />
              <div className="flex justify-between mt-2.5 text-[8px] text-bunker-mutado uppercase tracking-widest font-black">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="text-xs font-black uppercase text-bunker-mutado flex items-center gap-1.5">
                  Costos de Logística <AlertTriangle className="w-3.5 h-3.5 text-yellow-500" />
                </label>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="text-xs font-black uppercase text-bunker-mutado flex items-center gap-1.5">
                  Inflación Proyectada <TrendingDown className="w-3.5 h-3.5 text-[#FF4A4A]" />
                </label>
                <span className="text-xs font-black font-mono text-white">
                  {inflacion}%
                </span>
        ...
        {/* Output Panel */}
        <div className="lg:col-span-7 bg-bunker-panel p-6 rounded-[2rem] border border-white/5 shadow-2xl flex flex-col justify-center">
          <div className="flex items-center gap-3 mb-8">
            <ShieldCheck className="w-4 h-4 text-bunker-limon" />
            <h3 className="text-xs uppercase tracking-widest font-black text-white font-sans">Proyección de Supervivencia</h3>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-sans">
            {/* Corto Plazo */}
            <motion.div 
              className={`p-6 rounded-2xl border transition-all ${probCorto < 80 ? 'bg-red-950/20 border-red-500/30' : 'bg-black/30 border-white/5'}`}
              animate={simulating ? { scale: [1, 1.03, 1] } : {}}
              transition={{ duration: 0.3 }}
            >
              <div className="text-bunker-mutado text-[9px] font-black uppercase tracking-widest mb-1">6 Meses</div>
              ...
              <div className="mt-4 flex items-center gap-2">
                {probCorto < 80 ? (
                  <span className="text-[8px] text-[#FF4A4A] flex items-center gap-1 font-black uppercase tracking-wide bg-[#FF4A4A]/10 px-2.5 py-1 rounded">
                    ⚠️ Riesgo P.E.
                  </span>
                ) : (
                  <span className="text-[8px] text-bunker-limon flex items-center gap-1 font-black uppercase tracking-wide bg-bunker-limonDim px-2.5 py-1 rounded">
                    🛡️ Blindado
                  </span>
                )}
              </div>
            ...
            {/* Mediano Plazo */}
            <motion.div 
              className="p-6 rounded-2xl bg-black/30 border border-white/5"
              animate={simulating ? { scale: [1, 1.03, 1] } : {}}
              transition={{ duration: 0.3, delay: 0.1 }}
            >
              <div className="text-bunker-mutado text-[9px] font-black uppercase tracking-widest mb-1">12 Meses</div>
            ...
            >
              <div className="text-bunker-mutado text-[9px] font-black uppercase tracking-widest mb-1">24 Meses</div>
          ...
          {probCorto < 80 && (
            <motion.div 
              ...
              className="mt-6 p-4 rounded-xl bg-[#FF4A4A]/10 border border-[#FF4A4A]/30 flex items-start gap-3 font-sans"
            >
              <AlertTriangle className="w-5 h-5 text-[#FF4A4A] flex-shrink-0 mt-0.5" />
              <p className="text-xs text-[#FF4A4A] leading-relaxed font-black uppercase tracking-wide">
                El Punto de Estabilidad a corto plazo se proyecta por debajo del 80%. Se recomienda recortar gastos del Modo Disfrute y consolidar ahorro de emergencia inmediatamente.
              </p>
            </motion.div>
          )}
